    ...
    <div className="relative min-h-screen py-8 px-4">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        <motion.header className="text-center mb-8" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold mb-2 theme-title" style={{
            lineHeight: '2.0',
            paddingTop: '0.3em',
            overflow: 'visible',
          }}>
            <span style={{WebkitTextFillColor:'initial'}}>🔮</span> 解读结果
          </h1>
          {state.question && (
            <p className="text-sm mt-2" style={{color: 'var(--text-muted)'}}>你的问题：「{state.question}」</p>
          )}
        </motion.header>
          ...
          {state.type === 'cross' ? (
            <>
              {[3, 1, 0, 2, 4].map((idx) => (
                <div key={idx} className="flex flex-col items-center gap-1" style={{
                  ...
                  <span className="text-xs text-center leading-tight max-w-[120px]" style={{color: 'var(--highlight-text-strong)'}}>{state.positions[idx]}</span>
            ...
            state.cards.map((dc, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
                <span className="text-xs" style={{color: 'var(--highlight-text-strong)'}}>{state.positions[i]}</span>
        ...
        {/* 解读内容 */}
        <motion.div
          className="rounded-xl p-6 mb-8 theme-card"
        ...
        >
          {isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="animate-pulse" style={{color: 'var(--text-muted)'}}>正在解读牌面...</p>
            </div>
          ) : (
            <div className="leading-relaxed whitespace-pre-line text-base" style={{color: 'var(--text-primary)'}}>
              {interpretation}
            </div>
          )}
        </motion.div>
        ...
        <div className="text-center space-x-4">
          ...
          <motion.button
            className="px-6 py-2 rounded-lg text-white font-medium"
            style={{ background: 'var(--btn-secondary)', border: '1px solid var(--gold-border)', color: 'var(--text-primary)' }}
            onClick={() => navigate('/')}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
